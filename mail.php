...
##-----------------------------------------------------------------------------------------------------------------##
#
#  PHPメールプログラム【Mail02】　メールアドレス2重チェック版
#　改造や改変は自己責任で行ってください。
#	
...
$site_top = "https://koinu-step.com/";

// 管理者メールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
$to = "shop@koinu-step.com";
...
$thanksPage = "https://koinu-step.com/thanks";
...
$require = array('問い合わせ区分','姓','名','セイ','メイ','メールアドレス','電話番号','お問い合わせ内容','プライバシーポリシーに同意する');
...
// 以下の変更は知識のある方のみ自己責任でお願いします。（デザイン反映用のhtml部は除く）
...
<div class="_c-container">
	...
	<?php if($empty_flag == 1){ ?>
	    ...
	    <?php echo $errm; ?><br /><br /><div id="btn_area"><input class="btn btn_04" type="button" value=" 前画面に戻る " onClick="history.back()"></div>
	<?php }else{ ?>
		<h2>入力内容をご確認のうえ、「送信する」ボタンを押してください。</h2>
		<form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST">
		<table class="formTable">
		<?php echo confirmOutput($_POST);//入力内容を表示?>
		</table>
		<div id="btn_area"><input type="hidden" name="mail_set" value="confirm_submit">
		<input type="hidden" name="httpReferer" value="<?php echo h($_SERVER['HTTP_REFERER']);?>">
		<input class="btn btn_02" type="submit" value="　送信する　">
		<input class="btn btn_04" type="button" value="前画面に戻る" onClick="history.back()"></div>
		</form>
	<?php } ?>
</div><!-- /_c-container -->
...
function postToMail($arr){
	...
	foreach($arr as $key => $val) {
		...
		if($out != "confirm_submit" && $key != "httpReferer" && $key != $ConfirmEmail) {
